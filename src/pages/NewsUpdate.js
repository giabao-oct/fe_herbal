...
const NewsUpdate = () => {
  ...
  const handleSaveNews = async () => {
    if (!newsImg.files || imagePaths === "") {
      Swal.fire({
        icon: "warning",
        title: "Cảnh báo",
        text: "Vui lòng chọn một ảnh của tin tức.",
      });
      return;
    }
    ...
    if (selectedImages.length > 0) {
      urls = await uploadFiles(newsImg.files);
    } else {
    ...
    try {
      const response = await fetch(`${baseURL}update-news/${idtt}/`, {
        method: "PUT",
        headers: {
    ...
    <GlobalStyles>
      ...
      <div className={cx("wrapper")}>
        <div className={cx("container")}>
          <div className={cx("title")}>SỬA TIN TỨC</div>
